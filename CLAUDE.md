...
- **No test suite, no build step.** `node src/main.ts` runs it directly; `tsconfig.json` is type-check only (`npm run typecheck` = `tsc --noEmit`), extending `@tsconfig/node24` + `@tsconfig/strictest`. No bundler or test runner exists. Relative imports use `.ts` extensions (`./env.ts`, `./puppeteer-helpers.ts`).
