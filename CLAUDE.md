...
Puppeteer automation that logs into the Danish housing portal `aarhusbolig.dk` and declines every pending housing offer on the account. The whole program is one script: `src/main.ts`.
...
Env vars (parsed via the zod `ENV` schema atop `src/main.ts`):
